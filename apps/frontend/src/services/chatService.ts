...
export async function getContactsForUser() {
  ...
  const response = await fetch(
    `${backendUrl}/api/chat/contacts?userId=${supabaseUserId}`
  );
...
export async function getUnreadContactsAndGroupsForUser() {
  ...
  const contactsResponse = await fetch(
    `${backendUrl}/api/chat/contacts?userId=${supabaseUserId}&limit=4&isRead=false`
  );
  ...
  const groupsResponse = await fetch(
    `${backendUrl}/api/groups/getGroups?userId=${supabaseUserId}&limit=4&isRead=false`,
    {
      headers: {
        Authorization: `Bearer ${sessionData.session.access_token}`,
      },
...
export const deleteContact = async (contactId: number) => {
  try {
    ...
    console.log(
      `Attempting to delete contact with contactId: ${contactId} at URL: ${backendUrl}/api/chat/delete/${contactId}`
    );
